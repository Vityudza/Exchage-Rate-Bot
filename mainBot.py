...
def help(message):
    help_message = f'<strong>ВАЖЛИВО!!!</strong>\n<i>Всього є три кнопки, кожна з яких показує поточний курс тої чи іншої валюти.\nОднак, у цьому боті також присутній калькулятор, який переводить <strong><u>ВВЕДЕНУ ВАМИ</u></strong> валюту у гривні, а також переводить долари в євро (кільсті і через пробіл u_e) і євро в долари (кількість і через пробіл e_u). Також присутній калькулятор валютю. Наприклад "123 e_u"\n Для того, щоб перевести потрівно ввести кілкість цифрами і через пробіл валюту("usd" або "eur" або "rub").\nНаприкла "156 usd"</i>'
