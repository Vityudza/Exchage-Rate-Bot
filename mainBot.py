...
def Rate(message, url, rate):
    full_page = requests.get(url, headers=headers)
    soup = BeautifulSoup(full_page.content, 'html.parser')

    convert = soup.findAll("span", {"class": "DFlfde", "class": "SwHCTb", "data-precision": 2})
    final_message = f'<b>Курс:</b>\n<u>1 {rate} = {convert[0].text} гривень</u>'

    bot.send_message(message.chat.id, final_message,parse_mode='html')

def Calc(message, mess, url, rate):
    full_page = requests.get(url, headers=headers)
    ...
    final_message = f'<u>{mess[0]} {rate} = {result} гривень</u>'
...
def Main(message):
    ...
    if base[0] == 'долар':
        Rate(message, DOLLAR_UAH, 'долар')
    elif base[0] == 'євро':
        Rate(message, EURO_UAH, 'євро')
    elif base[0] == 'рублі':
        Rate(message, RUB_UAH, 'російський рубль')
    elif base[0].isdigit() and base[1] == 'usd':
        print(type(base))
        Calc(message, base, DOLLAR_UAH, 'доларів')
    elif base[0].isdigit() and base[1] == 'euro':
        Calc(message, base, EURO_UAH, 'євро')
    elif base[0].isdigit() and base[1] == 'rub':
        Calc(message, base, RUB_UAH, ' рублів')
    else:
